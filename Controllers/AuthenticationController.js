...
const {Account_activation_email} = require('../helpers/email_service');
...
const activate_account = async (req,res) => {
    ...
    if(token){
            JWT.verify(token,process.env.TOKEN_ACCOUNT_ACTIVATION, async(err,decoded)=>{
                ...
                if(err){
                    ...
                    if(userrr) return res.status(400).send('Account Already Activated!!');
...
//create JWT token
const token  = JWT.sign({user:user},process.env.TOKEN_SECRET,{expiresIn:'7d' //7 days login
});
...
const forgetPassword  = async(req,res) =>{
    ...
    User.findOne({email:email.email},(err,user) => {
        ...
        if(err || !user) { return res.status(400).send({error:'User with that email doesnt exist ' + email.email});}
...
try{
    ...
    (err, success) => {
      if(err) {
        ...
        return res.status(400).send({error:"Database connection error on password reset"});
      }else{
        
        Account_activation_email(emailData);
        return  res.status(200).send('please check your email to rest your password');
...
await User.findOneAndUpdate({ _id: _id},{password:hashpassword},(err,result)=>{
if(err){
...
client.verifyIdToken({idToken,audience:process.env.GOOGLE_CLIENT}).then(
   respons =>{
...
if(user){
    ...
    const {_id, email,firstname,lastname,profileUrl,gender,age} = user;
    return res.status(200).send({toke,user:{_id, email,firstname,lastname,profileUrl,gender,age}})
}else{
    //else if user doesn't exist we will save in the database
    let password = email + process.env.TOKEN_SECRET;
    user = new User({email,firstname,lastname,profileUrl,gender,age}); //create user object with email
        user.save((err,save)=>{
            if(err)
            {
                return res.status(400).send({error:"could not sign in"});
            }
            const token = JWT.sign({_id:DataCue._id},process.env.TOKEN_SECRET,{expiresIn:'7d'});
            const {_id,email,firstname,lastname,profileUrl,gender,age } = data;
            return res.status(200).send({
                token,
                user:{_id, email,firstname,lastname,profileUrl,gender,age}
              });
